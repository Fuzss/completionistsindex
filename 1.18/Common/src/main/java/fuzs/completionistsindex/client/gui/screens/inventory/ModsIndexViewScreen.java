...
//        CreativeModeTabs.tryRebuildTabContents(minecraft.player.connection.enabledFeatures(), false, minecraft.level.registryAccess());
        ...
        CreativeModeTab.TAB_SEARCH.fillItemList(displayItems);
        ...
        Map<String, List<ItemStack>> displayItemsByMod = items.stream()
                .collect(Collectors.groupingBy(item -> Registry.ITEM.getKey(item.getItem()).getNamespace(), Maps::newLinkedHashMap, Collectors.toList()));
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float tickDelta) {
        ...
        if (this.isLoading) {
            ...
            Component component = new TextComponent(LOADING_SYMBOLS[(int) (Util.getMillis() / 150L % (long) LOADING_SYMBOLS.length)]);
