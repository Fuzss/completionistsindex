...
public abstract class IndexViewScreen extends Screen {
   ...
   private static final MutableComponent PREVIOUS_PAGE_COMPONENT = new TranslatableComponent("createWorld.customize.custom.prev");
   private static final MutableComponent NEXT_PAGE_COMPONENT = new TranslatableComponent("createWorld.customize.custom.next");
   ...
   protected void init() {
      ...
      this.addRenderableWidget(DeferredTooltipRendering.setTooltip(new ImageButton(this.leftPos + 17, this.topPos + 11, 16, 13, 42, 202, 20, INDEX_LOCATION, 512, 256, button -> {
         this.onClose();
      }), CommonComponents.GUI_BACK));
      this.addRenderableWidget(DeferredTooltipRendering.setTooltip(new ImageButton(this.leftPos + 316 - 17 - 16, this.topPos + 11, 16, 13, 62, 202, 20, INDEX_LOCATION, 512, 256, button -> {
         statsSorting = statsSorting.cycle();
         DeferredTooltipRendering.setTooltip(button, statsSorting.component);
         this.rebuildPages();
      }), statsSorting.component));
      ...
      DeferredTooltipRendering.setTooltip(this.turnPageBackwards, PREVIOUS_PAGE_COMPONENT);
      ...
      DeferredTooltipRendering.setTooltip(this.turnPageForwards, NEXT_PAGE_COMPONENT);
   ...
   public void render(PoseStack poseStack, int mouseX, int mouseY, float tickDelta) {
      ...
      GuiComponent.blit(poseStack, this.leftPos, this.topPos, 0, 0, 316, 198, 512, 256);
   ...
   public static class IndexViewPage implements Widget {
      ...
      public static Entry modItemEntry(String modId, List<ItemStack> items, StatsCounter statsCounter, Font font) {
         ...
         if (modId.equals(ModsIndexViewScreen.ALL_ITEMS_PLACEHOLDER)) {
            modName = new TranslatableComponent("gui.all");
         } else {
            ...
            modName = new TextComponent(ModLoaderEnvironment.INSTANCE.getModName(modId).orElse(prettifyModId(modId)));
         }
      ...
      public static Entry statsItemEntry(ItemStack stack, StatsCounter statsCounter, Font font) {
         ...
         lines.add(new TextComponent("").append(stack.getItem().getName(stack)).withStyle(stack.getRarity().color));
         stack.getItem().appendHoverText(stack, null, lines, TooltipFlag.Default.NORMAL);
         if (pickedUp > 0) {
            lines.add(new TextComponent(String.valueOf(pickedUp)).append(" ").append(new TranslatableComponent("stat_type.minecraft.picked_up")).withStyle(ChatFormatting.BLUE));
         }
         if (crafted > 0) {
            lines.add(new TextComponent(String.valueOf(crafted)).append(" ").append(new TranslatableComponent("stat_type.minecraft.crafted")).withStyle(ChatFormatting.BLUE));
         }
      ...
      public static abstract class Entry {
         ...
         public void renderBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
            ...
            RenderSystem.setShaderTexture(0, INDEX_LOCATION);
            RenderSystem.setShader(GameRenderer::getPositionTexShader);
            GuiComponent.blit(poseStack, 0, 0, 120, 208, 18, 18, 512, 256);
            RenderSystem.setShaderTexture(0, INDEX_LOCATION);
            RenderSystem.setShader(GameRenderer::getPositionTexShader);
            GuiComponent.blit(poseStack, 124, 4, 120 + (this.collected ? 10 : 0), 198, 10, 10, 512, 256);
         }

         public void renderForeground(Minecraft minecraft, PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
            // from petrak from the collectors log mod, thanks!
            Matrix4f pose = poseStack.last().pose();
            poseStack = RenderSystem.getModelViewStack();
            poseStack.pushPose();
            poseStack.mulPoseMatrix(pose);
            RenderSystem.applyModelViewMatrix();
            minecraft.getItemRenderer().renderAndDecorateItem(this.item, 1, 1);
            poseStack.popPose();
            RenderSystem.applyModelViewMatrix();
         }
      }
      ...
      public static class ModItemEntry extends Entry {
         ...
         public void renderBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
            ...
            RenderSystem.setShaderTexture(0, INDEX_LOCATION);
            RenderSystem.setShader(GameRenderer::getPositionTexShader);
            GuiComponent.blit(poseStack, 24, 11, 140, 198, 91, 5, 512, 256);
            RenderSystem.setShaderTexture(0, INDEX_LOCATION);
            RenderSystem.setShader(GameRenderer::getPositionTexShader);
            GuiComponent.blit(poseStack, 24, 11, 140, 203, (int) (91 * this.collectionProgress), 5, 512, 256);
            if (this.isMouseOver(mouseX, mouseY)) {
               RenderSystem.setShaderTexture(0, INDEX_LOCATION);
               RenderSystem.setShader(GameRenderer::getPositionTexShader);
               GuiComponent.blit(poseStack, -2, -2, 316, 0, 140, 22, 512, 256);
            }
         }
